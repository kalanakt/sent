...
import sent_telegram_api as SentApi
import sent_telegram_types as SentTypes
import sent_telegram_util as SentUtil
...
class TelegramClient:
    def __init__(
        ...
        if self.threaded:
            self.worker_pool = SentUtil.ThreadPool()
    ...
    def set_webhook(self, url=None, certificate=None):
        return SentApi.set_webhook(self.token, url, certificate)

    def remove_webhook(self):
        return self.set_webhook()  # No params resets webhook

    def get_updates(self, offset=None, limit=None, timeout=20):
        json_updates = SentApi.get_updates(self.token, offset, limit, timeout)
        ret = []
        for ju in json_updates:
            ret.append(SentTypes.Update.de_json(ju))
        return ret
    ...
    def __threaded_polling(self, none_stop=False, interval=0, timeout=3):
        ...
        polling_thread = SentUtil.WorkerThread(name="PollingThread")
        or_event = SentUtil.OrEvent(
    ...
    def get_me(self):
        result = SentApi.get_me(self.token)
        return SentTypes.User.de_json(result)

    def log_out(self):
        return SentApi.log_out(self.token)

    def close(self):
        return SentApi.close(self.token)

    def download_file(self, file_path):
        return SentApi.download_file(self.token, file_path)
    ...
    def send_message(
        ...
        return SentTypes.Message.de_json(
            SentApi.send_message(
                self.token,
                chat_id,
                text,
                reply_to_message_id,
    ...
    def send_chat_action(self, chat_id, action, message_thread_id):
        return SentApi.send_chat_action(self.token, chat_id, action, message_thread_id)

    def get_user_profile_photos(self, user_id, offset=None, limit=None):
        result = SentApi.get_user_profile_photos(
            self.token, user_id, offset, limit)
        return SentTypes.UserProfilePhotos.de_json(result)

    def get_file(self, file_id):
        return SentTypes.File.de_json(SentApi.get_file(self.token, file_id))

    def ban_chat_member(self, chat_id, user_id, until_date=None, revoke_messages=None):
        return SentApi.ban_chat_member(
            self.token, chat_id, user_id, until_date=None, revoke_messages=None
        )

    def unban_chat_member(self, chat_id, user_id, only_if_banned=None):
        return SentApi.unban_chat_member(
            self.token, chat_id, user_id, only_if_banned=None
        )

    def restrict_chat_member(token, chat_id, user_id, permissions, until_date=None):
        return SentApi.restrict_chat_member(
    ...
    def promote_chat_member(
        ...
        return SentApi.promote_chat_member(
            self.token,
            chat_id,
            user_id,
            is_anonymous=None,
    ...
    def set_admininistrator_custom_title(self, chat_id, user_id, custom_title):
        return SentApi.set_admininistrator_custom_title(
            self.token, chat_id, user_id, custom_title
        )

    def ban_chat_sender_chat(self, chat_id, sender_chat_id):
        return SentApi.ban_chat_sender_chat(self.token, chat_id, sender_chat_id)

    def leave_chat(self, chat_id):
        return SentApi.leave_chat(self.token, chat_id)

    def unban_chat_sender_chat(self, chat_id, sender_chat_id):
        return SentApi.unban_chat_sender_chat(self.token, chat_id, sender_chat_id)

    def set_chat_permissions(self, chat_id, permissions):
        return SentApi.set_chat_permissions(self.token, chat_id, permissions)
    ...
    def create_chat_invite_link(
        ...
        return SentApi.create_chat_invite_link(
            self.token,
            chat_id,
            name=None,
            expire_date=None,
    ...
    def edit_chat_invite_link(
        ...
        return SentApi.edit_chat_invite_link(
            self.token,
            chat_id,
            name=None,
            expire_date=None,
    ...
    def revoke_chat_invite_link(self, chat_id, invite_link):
        return SentApi.revoke_chat_invite_link(self.token, chat_id, invite_link)

    def approve_chat_join_request(self, chat_id, user_id):
        return SentApi.approve_chat_join_request(self.token, chat_id, user_id)

    def decline_chat_join_request(self, chat_id, user_id):
        return SentApi.decline_chat_join_request(self.token, chat_id, user_id)

    def set_chat_photo(self, chat_id, photo):
        return SentApi.set_chat_photo(self.token, chat_id, photo)

    def delete_chat_photo(self, chat_id):
        return SentApi.delete_chat_photo(self.token, chat_id)

    def set_chat_title(self, chat_id, title):
        return SentApi.set_chat_title(self.token, chat_id, title)

    def set_chat_description(self, chat_id, description):
        return SentApi.set_chat_description(self.token, chat_id, description)

    def set_chat_sticker_set(self, chat_id, sticker_set_name):
        return SentApi.set_chat_sticker_set(self.token, chat_id, sticker_set_name)

    def delete_chat_sticker_set(self, chat_id):
        return SentApi.delete_chat_sticker_set(self.token, chat_id)

    def pin_chat_message(self, chat_id, message_id, disable_notification=None):
        return SentApi.pin_chat_message(
            self.token, chat_id, message_id, disable_notification=None
        )

    def unpin_chat_message(self, chat_id, message_id=None):
        return SentApi.unpin_chat_message(self.token, chat_id, message_id=None)

    def unpin_all_chat_messages(self, chat_id):
        return SentApi.unpin_all_chat_messages(self.token, chat_id)

    def get_chat(self, chat_id):
        return SentApi.get_chat(self.token, chat_id)

    def get_chat_administrators(self, chat_id):
        return SentApi.get_chat_administrators(self.token, chat_id)

    def get_chat_member_count(self, chat_id):
        return SentApi.get_chat_member_count(self.token, chat_id)

    def get_chat_member(self, chat_id):
        return SentApi.get_chat_member(self.token, chat_id)

    def get_forum_topic_icon_stickers(self):
        return SentApi.get_forum_topic_icon_stickers(self.token)

    def create_forum_topic(
        self, chat_id, name, icon_color=None, icon_custom_emoji_id=None
    ):
        return SentApi.create_forum_topic(
            self.token, chat_id, name, icon_color=None, icon_custom_emoji_id=None
        )

    def edit_forum_topic(
        self, chat_id, message_thread_id, name=None, icon_custom_emoji_id=None
    ):
        return SentApi.edit_forum_topic(
            self.token, chat_id, message_thread_id, name=None, icon_custom_emoji_id=None
        )

    def close_forum_topic(self, chat_id, message_thread_id):
        return SentApi.close_forum_topic(self.token, chat_id, message_thread_id)

    def reopen_forum_topic(self, chat_id, message_thread_id):
        return SentApi.reopen_forum_topic(self.token, chat_id, message_thread_id)

    def delete_forum_topic(self, chat_id, message_thread_id):
        return SentApi.delete_forum_topic(self.token, chat_id, message_thread_id)

    def unpin_all_forum_topic_messages(self, chat_id, message_thread_id):
        return SentApi.unpin_all_forum_topic_messages(
            self.token, chat_id, message_thread_id
        )

    def edit_genral_forum_topic(self, chat_id, name):
        return SentApi.edit_genral_forum_topic(self.token, chat_id, name)

    def close_genral_forum_topic(self, chat_id):
        return SentApi.close_genral_forum_topic(self.token, chat_id)

    def reopen_genral_forum_topic(self, chat_id):
        return SentApi.reopen_genral_forum_topic(self.token, chat_id)

    def hide_genral_forum_topic(self, chat_id):
        return SentApi.hide_genral_forum_topic(self.token, chat_id)

    def unhide_genral_forum_topic(self, chat_id):
        return SentApi.unhide_genral_forum_topic(self.token, chat_id)

    def set_my_commands(self, commands, scope=None, language_code=None):
        return SentApi.set_my_commands(
            self.token, commands, scope=None, language_code=None
        )

    def delete_my_commands(self, scope=None, language_code=None):
        return SentApi.delete_my_commands(self.token, scope=None, language_code=None)

    def get_my_commands(self, scope=None, language_code=None):
        return SentApi.get_my_commands(self.token, scope=None, language_code=None)

    def set_chat_menu_button(self, menu_button=None, chat_id=None):
        return SentApi.set_chat_menu_button(self.token, menu_button=None, chat_id=None)

    def get_chat_menu_button(self, chat_id=None):
        return SentApi.get_chat_menu_button(self.token, chat_id=None)

    def set_my_default_administrator_rights(self, rights=None, for_channels=None):
        return SentApi.set_my_default_administrator_rights(
            self.token, rights=None, for_channels=None
        )

    def get_my_default_administrator_rights(self, for_channels=None):
        return SentApi.get_my_default_administrator_rights(
            self.token, for_channels=None
        )
    ...
    def edit_message_text(
        ...
        results = SentApi.edit_message_text(
            self.token,
            text,
            chat_id=None,
            message_id=None,
        ...
        if results is bool:
            return results
        return SentTypes.Message.de_json(results)
    ...
    def edit_message_caption(
        ...
        return SentApi.edit_message_caption(
            self.token,
            caption,
            chat_id=None,
            message_id=None,
    ...
    def edit_message_media(
        self, media, chat_id=None, message_id=None, reply_markup=None
    ):
        return SentApi.edit_message_media(
            self.token, media, chat_id=None, message_id=None, reply_markup=None
        )

    def edit_message_replay_markup(
        self, chat_id=None, message_id=None, inline_message_id=None, reply_markup=None
    ):
        return SentApi.edit_message_replay_markup(
            self.token,
            chat_id=None,
            message_id=None,
            inline_message_id=None,
    ...
    def stop_poll(self, message_id, chat_id, reply_markup=None):
        return SentApi.stop_poll(self.token, message_id, chat_id, reply_markup=None)

    def delete_message(self, chat_id, message_id):
        return SentApi.delete_message(self.token, chat_id, message_id)

    def get_sticker_set(self, name):
        return SentApi.get_sticker_set(self.token, name)

    def get_custom_emoji_stickers(self, custom_emoji_ids):
        return SentApi.get_custom_emoji_stickers(self.token, custom_emoji_ids)

    def upload_sticker_file(self, user_id, png_sticker):
        return SentApi.upload_sticker_file(self.token, user_id, png_sticker)
    ...
    def create_new_sticker_set(
        ...
        return SentApi.create_new_sticker_set(
            self.token,
            user_id,
            name,
            title,
    ...
    def add_sticker_to_set(
        ...
        return SentApi.add_sticker_to_set(
            self.token,
            user_id,
            name,
            emojis,
    ...
    def set_sticker_position_in_set(self, sticker, position):
        return SentApi.set_sticker_position_in_set(self.token, sticker, position)

    def delete_sticker_from_set(self, sticker):
        return SentApi.delete_sticker_from_set(self.token, sticker)

    def set_sticker_set_thumb(self, name, user_id, thumb=None):
        return SentApi.set_sticker_set_thumb(self.token, name, user_id, thumb=None)
    ...
    def answer_inline_query(
        ...
        return SentApi.answer_inline_query(
            self.token,
            inline_query_id,
            results,
            cache_time=None,
    ...
    def create_invoice_link(
        ...
        return SentApi.create_invoice_link(
            self.token,
            title,
            description,
            payload,
    ...
    def answer_shipping_query(
        ...
        return SentApi.answer_shipping_query(
            self.token, shipping_query_id, ok, shipping_options=None, error_message=None
        )

    def answer_pre_checkout_query(self, pre_checkout_query_id, ok, error_message=None):
        return SentApi.answer_pre_checkout_query(
            self.token, pre_checkout_query_id, ok, error_message=None
        )

    def answer_callback_query(
        self, callback_query_id, text=None, show_alert=None, url=None, cache_time=None
    ):
        return SentApi.answer_callback_query(
            self.token,
            callback_query_id,
            text=None,
            show_alert=None,
    ...
    def set_passport_data_errors(self, user_id, errors):
        return SentApi.set_passport_data_errors(self.token, user_id, errors)
    ...
    def get_game_hight_score(
        self, user_id, chat_id=None, message_id=None, inline_message_id=None
    ):
        return SentApi.get_game_hight_score(
            self.token, user_id, chat_id=None, message_id=None, inline_message_id=None
        )
    ...
    def message_handler(
        self, commands=None, regexp=None, func=None, content_types=["text"]
    ):
    ...
    def add_message_handler(
        self, handler, commands=None, regexp=None, func=None, content_types=None
    ):
        ...
        handler_dict = {"function": handler, "filters": filters}
    ...
    def _test_message_handler(message_handler, message):
        for filter, filter_value in six.iteritems(message_handler["filters"]):
            if not TelegramClient._test_filter(filter, filter_value, message):
                return False
        return True
    ...
    def _test_filter(filter, filter_value, message):
        ...
        if filter == "commands":
            return (
                ...
                and SentUtil.extract_command(message.text) in filter_value
            )
        if filter == "lambda":
