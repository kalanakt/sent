...
import requests
from sent_telegram_types import JsonSerializable
from sent_telegram_util import is_string
...
def _make_request(
    token, method_name, method="get", params=None, files=None, base_url=API_URL
):
    """
...
def _check_result(method_name, result):
    ...
    try:
        ...
        msg = "Error : {0} \nThe server returned an invalid JSON response. Response body:\n[{1}]".format(
            e, result.text.encode("utf8")
        )
        raise ApiException(msg, method_name, result)

    if not result_json["ok"]:
        msg = "Error code: {0} Description: {1}".format(
...
def stop_message_live_location(
    token, chat_id, message_id=None, inline_message_id=None, reply_markup=None
):
